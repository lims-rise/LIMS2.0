...
class O2b_endetec_out_bs_model extends CI_Model
{

    public $table = 'obj2b_endetec3';
    ...
    // datatables
    function json() {
        $this->datatables->select('barcode_endetec, date_conduct, time_ecoli, volume_ecoli, total_coliforms, comments, lab, flag');
        $this->datatables->from('obj2b_endetec3');
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Update</button>'." 
                ".anchor(site_url('O2b_endetec_out_bs/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'barcode_sample');
        }
    ...
    function get_all()
    {
        ...
        return $this->db->get('obj2b_endetec3')->result();
    }
    ...
    //   }
      ...
      function validate1($id){
        ...
        $q = $this->db->query('
        SELECT * FROM (
        SELECT barcode_endetec FROM obj2b_subbs_endetec
        UNION ALL
        SELECT barcode_endetec FROM obj2b_subsd_endetec) x
        WHERE barcode_endetec NOT IN (SELECT barcode_endetec FROM obj2b_endetec3)
        AND barcode_endetec = "'.$id.'" 
        AND flag = 0
        ');
        $response = $q->result_array();
        return $response;
    ...
    //              AND x.barcodes1 NOT IN (SELECT barcode_endetec FROM obj2b_endetec1
