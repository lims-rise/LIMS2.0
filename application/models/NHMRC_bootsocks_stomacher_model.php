...
class NHMRC_bootsocks_stomacher_model extends CI_Model
{

    public $table = 'nhmrc_bs_stomacher';
    ...
    function json() {
        ...
        $this->datatables->from('nhmrc_bs_stomacher');
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Update</button>'." 
                ".anchor(site_url('NHMRC_bootsocks_stomacher/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting blood centrifuge : $1 ?\')"'), 'barcode_sample');
        }
    ...
    //     $this->datatables->from('nhmrc_subbs_endetec');
    ...
    //             ".anchor(site_url('NHMRC_bootsocks_stomacher/delete_det/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'barcode_sample');
    //     }
    ...
    function subjson2($id) {
        ...
        $this->datatables->from('nhmrc_subbs_idexx');
        $this->datatables->where('barcode_sample', $id);
        $this->datatables->where('flag', '0');
    ...
    function get_all()
    {
        ...
        g.elu_comments AS stomacher_elution_Micro1_comment,
        ...
        o.elu_comments AS stomacher_elution_Micro2_comment,
        ...
        s.elu_comments AS stomacher_elution_Moisture1_comment,
        ...
        LEFT JOIN nhmrc_bs_stomacher g ON a.barcode_sample=g.barcode_sample AND g.elution_no="Micro1"
        ...
        // return $this->db->get('nhmrc_bs_stomacher')->result();
    }
    ...
    function get_by_id_detail($id)
    {
        ...
        $this->db->where('flag', '0');
        return $this->db->get('nhmrc_bs_stomacher')->row();
    }
    ...
    {
        $this->db->insert('nhmrc_subbs_endetec', $data);
    }

    function insert_det2($data)
    {
        $this->db->insert('nhmrc_subbs_idexx', $data);
    }
    ...
    {
        $this->db->where('barcode_sample', $id);
        $this->db->update('nhmrc_subbs_endetec', $data);
    }

    function update_det2($id, $data)
    {
        $this->db->where('barcode_sample', $id);
        $this->db->update('nhmrc_subbs_idexx', $data);
    }
    ...
    {
        $this->db->where('id_bc', $id);
        $this->db->delete('nhmrc_bs_stomacher');
