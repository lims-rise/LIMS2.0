...
class NHMRC_bootsocks_after_model extends CI_Model
{

    public $table = 'nhmrc_bootsocks_after';
    ...
    // datatables
    function json() {
        $this->datatables->select('barcode_bootsocks, date_weighed, bootsock_weight_wet, comments, lab, flag');
        $this->datatables->from('nhmrc_bootsocks_after');
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Update</button>'." 
                ".anchor(site_url('NHMRC_bootsocks_after/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting O2B bootsocks sample : $1 ?\')"'), 'barcode_bootsocks');
        }
    ...
    //             ".anchor(site_url('NHMRC_bootsocks_after/delete_det/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting this sample detail ?\')"'), 'id_receipt_det');
    //     return $this->datatables->generate();
    // }
    ...
    function get_all()
    {
        ...
        return $this->db->get('nhmrc_bootsocks_after')->result();
    }
    ...
    //   }
      ...
      function validate1($id, $type){
        ...
        // $this->db->where('lab', $this->session->userdata('lab'));
        $this->db->where('flag', '0');
        $q = $this->db->get('nhmrc_bootsocks_after');
        $response = $q->result_array();
        return $response;
