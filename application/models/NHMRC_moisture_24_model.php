...
class NHMRC_moisture_24_model extends CI_Model
{

    public $table = 'nhmrc_moisture2';
    ...
    function json() {
        ...
        $this->datatables->from('nhmrc_moisture2');
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Update</button>'." 
                ".anchor(site_url('NHMRC_moisture_24/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'barcode_foil');
        }
    ...
    function get_all()
    {
        ...
        return $this->db->get('nhmrc_moisture2')->result();
    }
    ...
    //   }
      ...
      function validate1($id){
        ...
        $q = $this->db->query('
        SELECT barcode_foil FROM nhmrc_moisture1
        WHERE barcode_foil = "'.$id.'"
        AND barcode_foil NOT IN (SELECT barcode_foil FROM nhmrc_moisture2)
        AND flag = 0 
        ');        
        $response = $q->result_array();
        return $response;
