...
class DNA_extraction_model extends CI_Model
{
    ...
    //     ) g ON a.barcode_dna=g.barcode_sample AND g.lab = "'.$lab.'"
    //     LEFT JOIN ref_location_80 d on g.id_location_80=d.id_location_80 AND d.lab = "'.$lab.'"                             									
	// 			WHERE a.lab = "'.$lab.'"
    //     AND a.flag = 0 
    //     ORDER BY a.date_extraction
    //     ');
    ...
    function get_all()
    {
        ...
        FROM dna_extraction a
        LEFT JOIN ref_person b ON a.id_person = b.id_person
        LEFT JOIN (SELECT * FROM v_dna_all_sample
					WHERE lab = "'.$lab.'"
					) c ON a.barcode_sample=c.barcode
        LEFT JOIN (
					SELECT a.id, a.date_in, a.time_in, a.barcode_sample, a.lab, a.id_location_80
