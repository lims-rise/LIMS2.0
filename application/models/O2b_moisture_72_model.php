...
class O2b_moisture_72_model extends CI_Model
{

    public $table = 'obj2b_moisture4';
    ...
    function json() {
        ...
        $this->datatables->from('obj2b_moisture4');
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Update</button>'." 
                ".anchor(site_url('O2b_moisture_72/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'barcode_foil');
        }
    ...
    function get_all()
    {
        ...
        return $this->db->get('obj2b_moisture4')->result();
    }
    ...
    //   }
      ...
      function validate1($id){
        ...
        $q = $this->db->query('
        SELECT barcode_foil FROM obj2b_moisture3
        WHERE barcode_foil = "'.$id.'"
        AND difference = "CONTINUE"
        AND barcode_foil NOT IN (SELECT barcode_foil FROM obj2b_moisture4)
        AND flag = 0 
        ');        
        $response = $q->result_array();
        return $response;
