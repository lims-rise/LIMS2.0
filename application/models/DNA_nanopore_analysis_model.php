...
class DNA_nanopore_analysis_model extends CI_Model
{

    public $table = 'DNA_nanopore_analysis';
    ...
    function json() { 
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Update</button>'." 
                ".anchor(site_url('DNA_nanopore_analysis/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'barcode_dna');
        }
    ...
    //   }
      ...
      function validate1($id){
        $sql_asys = $this->db->query('SELECT barcode_dna FROM dna_sample_analysis
            WHERE analysis_type = "Nanopore"
            AND flag = 0
            AND barcode_dna = "'.$id.'"
            ');
      ...
      function validate2($id1,$id2){
        $sql_asys = $this->db->query('SELECT * 
        FROM dna_nanopore_analysis
