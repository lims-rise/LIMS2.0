...
class SE_sample_result_model extends CI_Model
{

    public $table = 'se_result';
    ...
    // datatables
    function json() {
        $this->datatables->select('barcode_sample, date_analysis, analyte, result, units, person, comments');
        $this->datatables->from('se_result');
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Update</button>'." 
                ".anchor(site_url('SE_sample_result/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'barcode_sample');
       }
    ...
    function get_all()
    {
        $this->db->order_by('date_analysis', 'ASC');
        $this->db->where('lab', $this->session->userdata('lab'));
        $this->db->where('flag', '0');
        return $this->db->get('se_result')->result();
    }
