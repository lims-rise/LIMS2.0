...
class O2a_mosquito_identifications extends CI_Controller
{
    ...
    public function save() 
    {
        ...
        $id = strtoupper($this->input->post('bar_storagebag',TRUE));
        $dt = new DateTime();

        if ($mode=="insert"){
            $data = array(
            'bar_storagebag' => strtoupper($this->input->post('bar_storagebag',TRUE)),
    ...
    $datenow=date("Ymd");
    $fileName = 'O2A_Mosquito_Identifications_'.$datenow.'.csv';
