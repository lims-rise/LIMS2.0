...
class Ref_allsample extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Ref_allsample_model');
    ...
    {
        // $this->load->model('Ref_allsample_model');
        // $data['person'] = $this->Ref_allsample_model->getLabtech();
        // $data['type'] = $this->Ref_allsample_model->getSampleType();
        // $this->template->load('template','Ref_allsample/index', $data);
        $this->template->load('template','Ref_allsample/index');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->Ref_allsample_model->json();
    }
    ...
    public function save() 
    {
        ...
        if ($mode=="insert"){
            ...
            $this->Ref_allsample_model->insert($data);
            ...
            $this->Ref_allsample_model->update($id, $data);
            $this->session->set_flashdata('message', 'Create Record Success');    
        }

        redirect(site_url("Ref_allsample"));
    }

    public function delete($id) 
    {
        $row = $this->Ref_allsample_model->get_by_id($id);
        ...
        if ($row) {
            // $this->Ref_allsample_model->delete($id);
            $this->Ref_allsample_model->update($id, $data);
    ...
    public function valid_bs() 
    {
        ...
        // echo $id;
        $data = $this->Ref_allsample_model->validate1($id);
    ...
    public function excel()
    {
        ...
        $rdeliver = $this->Ref_allsample_model->get_all();
...
/* End of file Ref_allsample.php */
/* Location: ./application/controllers/Ref_allsample.php */
