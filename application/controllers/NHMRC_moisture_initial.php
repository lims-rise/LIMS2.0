...
class NHMRC_moisture_initial extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model(['NHMRC_moisture_initial_model']);
    ...
    {
        // $this->load->model('NHMRC_moisture_initial_model');
        // $data['person'] = $this->NHMRC_moisture_initial_model->getLabtech();
        // $data['type'] = $this->NHMRC_moisture_initial_model->getSampleType();
        ...
        $this->template->load('template','NHMRC_moisture_initial/index');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->NHMRC_moisture_initial_model->json();
    }
    ...
    public function save() 
    {
        ...
        if ($mode=="insert"){
            ...
            $this->NHMRC_moisture_initial_model->insert($data);
            ...
            $this->NHMRC_moisture_initial_model->update($id, $data);
            $this->session->set_flashdata('message', 'Create Record Success');    
        }

        redirect(site_url("NHMRC_moisture_initial"));
    }

    public function delete($id) 
    {
        $row = $this->NHMRC_moisture_initial_model->get_by_id($id);
        ...
        if ($row) {
            // $this->NHMRC_moisture_initial_model->delete($id);
            $this->NHMRC_moisture_initial_model->update($id, $data);
    ...
    //     $data = $this->NHMRC_moisture_initial_model->load_freez($id);
    //     header('Content-Type: application/json');
    ...
    //     $data = $this->NHMRC_moisture_initial_model->get_freez($id1, $id2, $id3, $id4);
    //     header('Content-Type: application/json');
    //     echo json_encode($data);
    // }
    ...
    {
        $id = $this->input->get('id1');
        $data = $this->NHMRC_moisture_initial_model->validate1($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    {
        $id = $this->input->get('id1');
        $data = $this->NHMRC_moisture_initial_model->validate2($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function excel()
    {
        ...
        $rdeliver = $this->NHMRC_moisture_initial_model->get_all();
    ...
    $datenow=date("Ymd");
    $fileName = 'NHMRC_moisture_initial_'.$datenow.'.csv';
...
/* End of file NHMRC_moisture_initial.php */
/* Location: ./application/controllers/NHMRC_moisture_initial.php */
