...
class O2b_endetec_out_bs extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('O2b_endetec_out_bs_model');
    ...
    {
        // $this->load->model('O2b_endetec_out_bs_model');
        // $data['person'] = $this->O2b_endetec_out_bs_model->getLabtech();
        // $data['type'] = $this->O2b_endetec_out_bs_model->getSampleType();
        $this->template->load('template','O2b_endetec_out_bs/index');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->O2b_endetec_out_bs_model->json();
    }

    public function save() 
    {
        $mode = $this->input->post('mode',TRUE);
        $id = strtoupper($this->input->post('barcode_endetec',TRUE));
        $dt = new DateTime();
        ...
        if ($mode=="insert"){
            ...
            $this->O2b_endetec_out_bs_model->insert($data);
            ...
            $this->O2b_endetec_out_bs_model->update($id, $data);
            $this->session->set_flashdata('message', 'Create Record Success');    
        }

        redirect(site_url("O2b_endetec_out_bs"));
    }

    public function delete($id) 
    {
        $row = $this->O2b_endetec_out_bs_model->get_by_id($id);
        ...
        if ($row) {
            // $this->O2b_endetec_out_bs_model->delete($id);
            $this->O2b_endetec_out_bs_model->update($id, $data);
    ...
    public function valid_bs1() 
    {
        $id = $this->input->get('id1');
        $data = $this->O2b_endetec_out_bs_model->validate1($id);
        header('Content-Type: application/json');
    ...
    //     $data = $this->O2b_endetec_out_bs_model->validate2($id, $id2);
    //     header('Content-Type: application/json');
    //     echo json_encode($data);
    // }
    ...
    public function excel()
    {
        ...
        $sheet->setCellValue('E1', "Total_coliforms_CFU/volume_added");
        $sheet->setCellValue('F1', "Comments");
        // $sheet->getStyle('A1:H1')->getFont()->setBold(true); // Set bold kolom A1

        // Panggil function view yang ada di SiswaModel untuk menampilkan semua data siswanya
        $rdeliver = $this->O2b_endetec_out_bs_model->get_all();
        ...
        foreach($rdeliver as $data){ // Lakukan looping pada variabel siswa
          ...
          $sheet->setCellValue('E'.$numrow, $data->total_coliforms);
    ...
    $datenow=date("Ymd");
    $fileName = 'O2B_Endetec_Out_(B-S)_'.$datenow.'.csv';
...
/* End of file O2b_endetec_out_bs.php */
/* Location: ./application/controllers/O2b_endetec_out_bs.php */
