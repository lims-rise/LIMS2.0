...
class SE_sample_result extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('SE_sample_result_model');
    ...
    {
        // $this->load->model('SE_sample_result_model');
        // $data['person'] = $this->SE_sample_result_model->getLabtech();
        // $data['type'] = $this->SE_sample_result_model->getSampleType();
        // $this->template->load('template','SE_sample_result/index', $data);
        $this->template->load('template','SE_sample_result/index');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->SE_sample_result_model->json();
    }
    ...
    public function save() 
    {
        ...
        if ($mode=="insert"){
            ...
            'analyte' => $this->input->post('analyte',TRUE),
            ...
            $this->SE_sample_result_model->insert($data);
            ...
            $this->SE_sample_result_model->update($id, $data);
            $this->session->set_flashdata('message', 'Create Record Success');    
        }

        redirect(site_url("SE_sample_result"));
    }

    public function delete($id) 
    {
        $row = $this->SE_sample_result_model->get_by_id($id);
        ...
        if ($row) {
            // $this->SE_sample_result_model->delete($id);
            $this->SE_sample_result_model->update($id, $data);
    ...
    public function valid_bs() 
    {
        ...
        // echo $id;
        $data = $this->SE_sample_result_model->validate1($id);
    ...
    public function excel()
    {
        ...
        foreach($rdeliver as $data){ // Lakukan looping pada variabel siswa
          ...
          $sheet->setCellValue('C'.$numrow, $data->analyte);
    ...
    $datenow=date("Ymd");
    $fileName = 'SE_Sample_Result_'.$datenow.'.csv';
...
/* End of file SE_sample_result.php */
/* Location: ./application/controllers/SE_sample_result.php */
