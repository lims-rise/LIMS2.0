...
class NHMRC_idexx_out extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('NHMRC_idexx_out_model');
    ...
    {
        // $this->load->model('NHMRC_idexx_out_model');
        // $data['person'] = $this->NHMRC_idexx_out_model->getLabtech();
        // $data['type'] = $this->NHMRC_idexx_out_model->getSampleType();
        $this->template->load('template','NHMRC_idexx_out/index');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->NHMRC_idexx_out_model->json();
    }

    public function save() 
    {
        $mode = $this->input->post('mode',TRUE);
        $id = strtoupper($this->input->post('barcode_colilert',TRUE));
        $dt = new DateTime();
        ...
        if ($mode=="insert"){
            ...
            $this->NHMRC_idexx_out_model->insert($data);
            ...
            $this->NHMRC_idexx_out_model->update($id, $data);
            $this->session->set_flashdata('message', 'Create Record Success');    
        }

        redirect(site_url("NHMRC_idexx_out"));
    }

    public function delete($id) 
    {
        $row = $this->NHMRC_idexx_out_model->get_by_id($id);
        ...
        if ($row) {
            // $this->NHMRC_idexx_out_model->delete($id);
            $this->NHMRC_idexx_out_model->update($id, $data);
    ...
    public function load_dilution() 
    {
        ...
        $data = $this->NHMRC_idexx_out_model->load_dilution($id);
        header('Content-Type: application/json');
    ...
    public function data_chart() 
    {
        ...
        $data = $this->NHMRC_idexx_out_model->data_chart($id1, $id2);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    {
        $id = $this->input->get('id1');
        $data = $this->NHMRC_idexx_out_model->validate1($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    //     $id = $this->input->get('id1');
    //     $id2 = $this->input->get('id2');
    //     $data = $this->NHMRC_idexx_out_model->validate2($id, $id2);
    //     header('Content-Type: application/json');
    //     echo json_encode($data);
    // }
    ...
    public function excel()
    {
        ...
        $rdeliver = $this->NHMRC_idexx_out_model->get_all();
    ...
    $datenow=date("Ymd");
    $fileName = 'NHMRC_idexx_out_(All)_'.$datenow.'.csv';
...
/* End of file NHMRC_idexx_out.php */
/* Location: ./application/controllers/NHMRC_idexx_out.php */
