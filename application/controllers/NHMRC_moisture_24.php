...
class NHMRC_moisture_24 extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model(['NHMRC_moisture_24_model']);
    ...
    {
        // $this->load->model('NHMRC_moisture_24_model');
        // $data['person'] = $this->NHMRC_moisture_24_model->getLabtech();
        // $data['type'] = $this->NHMRC_moisture_24_model->getSampleType();
        ...
        $this->template->load('template','NHMRC_moisture_24/index');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->NHMRC_moisture_24_model->json();
    }
    ...
    public function save() 
    {
        ...
        if ($mode=="insert"){
            ...
            $this->NHMRC_moisture_24_model->insert($data);
            ...
            $this->NHMRC_moisture_24_model->update($id, $data);
            $this->session->set_flashdata('message', 'Create Record Success');    
        }

        redirect(site_url("NHMRC_moisture_24"));
    }

    public function delete($id) 
    {
        $row = $this->NHMRC_moisture_24_model->get_by_id($id);
        ...
        if ($row) {
            // $this->NHMRC_moisture_24_model->delete($id);
            $this->NHMRC_moisture_24_model->update($id, $data);
    ...
    //     $data = $this->NHMRC_moisture_24_model->load_freez($id);
    //     header('Content-Type: application/json');
    ...
    //     $data = $this->NHMRC_moisture_24_model->get_freez($id1, $id2, $id3, $id4);
    //     header('Content-Type: application/json');
    //     echo json_encode($data);
    // }
    ...
    {
        $id = $this->input->get('id1');
        $data = $this->NHMRC_moisture_24_model->validate1($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function excel()
    {
        ...
        $sheet->setCellValue('C1', "Dry_weight(24h)");
        $sheet->setCellValue('D1', "Comments");
        // $sheet->getStyle('A1:H1')->getFont()->setBold(true); // Set bold kolom A1

        // Panggil function view yang ada di SiswaModel untuk menampilkan semua data siswanya
        $rdeliver = $this->NHMRC_moisture_24_model->get_all();
        ...
        foreach($rdeliver as $data){ // Lakukan looping pada variabel siswa
          ...
          $sheet->setCellValue('C'.$numrow, $data->dry_weight24);
    ...
    $datenow=date("Ymd");
    $fileName = 'NHMRC_moisture_24h_'.$datenow.'.csv';
...
/* End of file NHMRC_moisture_24.php */
/* Location: ./application/controllers/NHMRC_moisture_24.php */
